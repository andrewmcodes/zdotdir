...
- The `main() { ... }; main "$@"` pattern (see `functions/grecent`) is the convention **with known exceptions**: 5 of 9 use it (`cached-eval`, `calculate_actions_stats`, `fetch_action_stats`, `grecent`, `optdiff`); `funcs`, `is-macos`, `os` and `bench-startup` are flat scripts. Use `main()` when the function has locals to scope or arguments to validate; a two-line predicate like `is-macos` doesn't need it.
