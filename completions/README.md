...
Adding or removing a file here changes `$fpath`, which `conf.d/04-completion.zsh` detects — the completion dump is rebuilt on the next shell, so new completions work immediately.

Two things that do *not* belong here:

- **Aliases.** Use the `compdef name=service` form in `conf.d/04-completion.zsh` instead (e.g. `g=git`). The `#compdef name=service` file-tag form does not exist — `compinit`'s file tag only accepts names, `-p`/`-P` patterns, `-k` and `-K`.
